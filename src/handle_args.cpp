...
// License
    else if(it == "-l" || it == "--license")
    {
      ...
      {
        std::printf("tng error : You can't address license file more then one time per command, but you  can use .config file for setting license PATH's that can help you find you license when it is not in this directory\n");
        EXIT_FAILURE;
      }
      else
      {
